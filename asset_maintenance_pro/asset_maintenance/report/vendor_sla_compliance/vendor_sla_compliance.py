def execute(filters=None):
    ...
    columns = [
        ...
        {"label":"Jobs","fieldname":"job_count","fieldtype":"Int","width":80},
        {"label":"Visits Used","fieldname":"visits_used","fieldtype":"Int","width":100},
        {"label":"Visits Included","fieldname":"visits_included","fieldtype":"Int","width":120},
    ]
    ...
    for c in contracts:
        ...
        avg_actual = (sum(float(j.vendor_response_time or 0) for j in jobs) / len(jobs)) if jobs else 0
        sla = c.response_time_hours or 1
        comp = min(100, round(sla / avg_actual * 100, 1)) if avg_actual else 100
        data.append({
            "vendor": c.vendor, "contract": c.name,
            "contract_type": c.contract_type,
            "sla_response": sla, "avg_actual": round(avg_actual, 2),
            "compliance_pct": comp, "job_count": len(jobs),
