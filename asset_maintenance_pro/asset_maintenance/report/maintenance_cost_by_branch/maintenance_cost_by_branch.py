def execute(filters=None):
    ...
    vendor_rows = frappe.db.sql("""
        SELECT wo.branch,
               SUM(COALESCE(wo.total_cost,0)) AS vendor_cost,
        ...
        GROUP BY wo.branch
    """, as_dict=True)
    vendor_map = {r.branch: r for r in vendor_rows}

    data = []
    for r in rows:
        vc = float(vendor_map.get(r.branch, frappe._dict()).get("vendor_cost") or 0)
        downtime = float(vendor_map.get(r.branch, frappe._dict()).get("total_downtime") or 0)
        parts = float(r.parts_cost or 0)
        total = parts + vc
        count = r.total_requests or 1
        data.append({
            "branch": r.branch,
            "corrective_count": r.corrective_count or 0,
            "preventive_count": r.preventive_count or 0,
            "parts_cost": parts, "vendor_cost": vc, "total_cost": total,
